...
/*   Updated: 2024/11/25 14:05:34 by gueberso         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	test_for_bonus_part_1(void)
{
    ...
    ft_printf("\n\n\ntest_for_bonus_part_1 :\n\n\n");
	ft_printf("TESTS WITH NO #      %%x    '%x'\n", x);
	ft_printf("TESTS WITH NO #      %%X    '%X'\n", X);
	ft_printf("TESTS WITH    #      %%#x   '%#x'\n", x);
	ft_printf("TESTS WITH    #      %%#X   '%#X'\n", X);
	ft_printf("TESTS FOR MULTIPLE # %%###x '%###x'\n", x);
	ft_printf("TESTS FOR MULTIPLE # %%###X '%###X'\n\n\n", X);

	ft_printf("TESTS WITH NO #      %%vnull    '%x'\n", vnull);
	ft_printf("TESTS WITH    #      %%#vnull   '%#x'\n", vnull);
	ft_printf("TESTS FOR MULTIPLE # %%###vnull '%###x'\n\n", vnull);
}
...
void	test_for_bonus_part_2(void) // FLAG ' ' (ESPACE) --- FORMAT : d ; i
{
    ...
    ft_printf("\n\n\ntest_for_bonus_part_2 :\n\n\n");
	ft_printf("                         TESTS WITH no ' ' %%d : '%d'\n", d);
	ft_printf("                        TESTS WITH one ' ' %%d : '% d'\n", d);
	ft_printf("                   TESTS WITH multiple '    ' %%d : '%    d'\n", d);
    ft_printf("TESTS WITH multiple (before and after) '    d  '    ' %%d : '%    d  '    ''\n", d);
    ft_printf("TESTS WITH multiple identifier and flags (before and after) '    d '    ' %%d : '%    d '    '%  d'\n", d, D);
...
void    test_for_bonus_part_4(void) // FLAG '+ ' (ESPACE) --- format : d ; i
{
    ...
    ft_printf("\n\nTESTS WITH ' ' (SPACE) PRIOR TO '+'\n\n");
    ft_printf("shouldnt print anything since flag ' ' is ignored when flag '+' is present prior\n\n");
    ft_printf("                         TESTS WITH no ' +' %%d : '%d'\n", d);
    ft_printf("TESTS WITH multiple identifier and flags (before and after) '+++   d++   '    ' %%d : '%+++d++'    '%+d'\n", d, D);
}
